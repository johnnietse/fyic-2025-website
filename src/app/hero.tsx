...
function Hero() {
  ...
  // Video autoplay and loop handling
  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;

    // Attempt autoplay immediately (muted by default)
    const playPromise = video.play().catch(e => {
      console.log("Autoplay failed, trying again:", e);
      // Some browsers require a second attempt
      setTimeout(() => video.play().catch(e => console.log("Second autoplay attempt failed:", e)), 500);
    });

    // Smooth loop transition handler
    const handleLoop = () => {
      if (video.duration > 0 && video.currentTime > video.duration - 1.0) {
        const fadeIn = () => {
          setTransitionOpacity((prev) => {
            const newOpacity = Math.min(prev + 0.02, 0.7);
            if (newOpacity < 0.7) {
              requestAnimationFrame(fadeIn);
            } else {
              video.currentTime = 0;
